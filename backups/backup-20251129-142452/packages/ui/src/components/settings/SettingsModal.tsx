...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const savedKey = sessionStorage.getItem('mini-ide-api-key') || '';
      const savedModel = sessionStorage.getItem('mini-ide-model') || 'gpt-4o';
      const savedUrl = sessionStorage.getItem('mini-ide-base-url') || 'https://api.openai.com/v1';
  ...
  const handleSave = () => {
    sessionStorage.setItem('mini-ide-api-key', apiKey);
    sessionStorage.setItem('mini-ide-model', model);
    sessionStorage.setItem('mini-ide-base-url', baseUrl);
