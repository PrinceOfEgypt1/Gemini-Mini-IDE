...
| MC-005 | CI/CD | Coverage gate real no CI | Antes houve superdeclaracao de enforcement | Thresholds coerentes no root/pacotes e falha real no CI | COMPLETO | - | Rodar testes com coverage no CI | Nao | Rodada 17 | **ATUALIZADO Rodada 17:** Todos os 5 pacotes com thresholds locais: shared 80/80/50/80, server 40/55/30/40, cli 40/80/80/40, ui 20/60/35/20, analysis-agent 15/70/45/15. Root/global 25/15/25/25 como safety net |
...
| MC-030 | Arquitetura | Unificacao arquitetural do server (Operacao Fonte Unica) | index.ts monolitico com 602 linhas concentrando entrypoint, rotas, servicos e orquestracao | Entrypoint fino (~78 linhas); rotas extraidas em 3 modulos; servico de gestao de agentes isolado; fonte unica de verdade para conversacao explicitada | COMPLETO | - | Pipeline 6/6 OK; 441 testes passando; lint/typecheck/build verdes; contratos de API inalterados | Nao | Rodada 16 | **EVIDENCIA:** index.ts 602->78 linhas; 3 modulos de rotas (analysis, conversation, esaa); 1 modulo de servico (agent-manager); conversation.routes.ts e a fonte unica de verdade |
| MC-031 | Testes | Governanca de coverage institucionalizada (Operacao Guarda de Cobertura) | UI e analysis-agent sem thresholds locais; coverage nao governada nesses pacotes | Thresholds locais por pacote em TODOS os 5 pacotes; policy documentada e auditavel | COMPLETO | - | Pipeline 6/6 OK; 441 testes passando; thresholds nao quebram build; todos os pacotes com coverage governance | Nao | Rodada 17 | **EVIDENCIA:** UI thresholds 20/60/35/20 (abaixo do atual 22.23/65.82/38.80/22.23); analysis-agent thresholds 15/70/45/15 (abaixo do atual 16.66/73.42/51.25/16.66); root/global mantido em 25/15/25/25; 5/5 pacotes com thresholds locais |
| MC-032 | Testes | Blindagem do core do analysis-agent (Operacao Escudo do Nucleo) | orchestrator-interactive.ts (680 linhas, 0% coverage); prompt-orchestrator.ts e cache.service.ts com 0% coverage; core do analysis-agent sem testes | Testes reais cobrindo orchestrator-interactive (95.58%), prompt-orchestrator (98.6%), cache.service (91%); 72 testes novos | COMPLETO | - | Pipeline 6/6 OK; 513 testes passando; analysis-agent coverage 16.73%->22.93% stmts, 51.57%->62.43% funcs | Nao | Rodada 18 | **EVIDENCIA:** 3 novos arquivos de teste; orchestrator-interactive.ts 0%->95.58% stmts/94.11% funcs; prompt-orchestrator.ts 0%->98.6% stmts/100% funcs; cache.service.ts 0%->91% stmts/100% funcs; agent.test.ts/ESAA exclusoes auditadas — bloqueio tecnico documentado |
...
| 2026-03-15 | Rodada 16 - Fase 5 | Operacao Fonte Unica — unificacao arquitetural do server | IA | MC-030 criado como COMPLETO; index.ts 602->78 linhas; 4 novos modulos criados; contratos de API preservados |
| 2026-03-15 | Rodada 17 - Fase 5 | Operacao Guarda de Cobertura — governanca de coverage institucionalizada | IA | MC-031 criado como COMPLETO; MC-005 promovido a COMPLETO; thresholds adicionados a ui e analysis-agent; 5/5 pacotes com coverage governance |
| 2026-03-15 | Rodada 18 - Fase 5 | Operacao Escudo do Nucleo — blindagem do core do analysis-agent | IA | MC-032 criado como COMPLETO; orchestrator-interactive 0%->95.58%; prompt-orchestrator 0%->98.6%; cache.service 0%->91%; 72 testes novos; 513 testes totais |
