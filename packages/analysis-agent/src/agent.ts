...
export class AnalysisAgent extends EventEmitter {
  ...
  constructor(options: AnalysisAgentOptions) {
    super();
    this.options = options;
    this.promptOrchestrator = new PromptOrchestrator();
    this.codeGenerationService = new CodeGenerationService(options.incrementalClient, this.promptOrchestrator, this.validationService);
  ...
  private async callLLM<T>(prompt: { system: string; user: string }, schema: z.ZodSchema<T>): Promise<T> {
