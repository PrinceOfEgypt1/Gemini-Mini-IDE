...
# ==============================================================================
# SCRIPT: 01_apply_quick_wins.sh
# DESCRIÇÃO: Aplica correções de latência (timeout, delay removal) e 
#            consistência (seed, temp 0) conforme HU-MINI-IDE-PERF-001.
# AUTOR: Mini-IDE Engine Team
# DATA: 30/11/2025
# ==============================================================================

echo ">>> Iniciando aplicação de Quick Wins (Performance & Consistência)..."

# ------------------------------------------------------------------------------
# 1. Atualizando packages/analysis-agent/src/agent.ts
# ------------------------------------------------------------------------------
echo ">>> Reescrevendo packages/analysis-agent/src/agent.ts..."
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, _budgetContext?: BudgetContext): Promise<AgentResult> {
    ...
    logger.info("[Agent v4.1] Iniciando Pipeline Otimizado");
    ...
    try {
      ...
      // Otimização: Paralelização dentro do batch ainda é possível, mas removemos o delay artificial
      const batchSize = 5;
      ...
      for (let i = 0; i < manifest.length; i += batchSize) {
        ...
        // Motivo: HU-MINI-IDE-PERF-001 - Remover delay artificial
      }
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Generated via Agent v4.1 (Optimized)" }
      };
  ...
  private async generateTextResponse(prompt: string): Promise<string> {
    const completion = await this.client.chat.completions.create({
      ...
      // HU-MINI-IDE-PERF-001: Timeout explícito
    }, { timeout: 30000 });
  ...
  private async runArchitectureStep(_prompt: string, productPlan: ProductPlan): Promise<Architecture> {
  ...
  private async callLLM<T>(
    ...
    try {
      const completion = await this.client.chat.completions.create({
        ...
        // HU-MINI-IDE-PERF-001: Consistência
        temperature: 0.0,
        seed: 42
      }, {
        // HU-MINI-IDE-PERF-001: Timeout de 30s
        timeout: 30000 
      });
...
# ------------------------------------------------------------------------------
# 2. Atualizando packages/server/src/index.ts
# ------------------------------------------------------------------------------
echo ">>> Reescrevendo packages/server/src/index.ts..."
...
echo ">>> Executando verificação de build e lint (Gate de Qualidade)..."

# Validação Obrigatória (Pipeline Checklist)
pnpm lint || { echo "❌ Lint falhou"; exit 1; }
pnpm typecheck || { echo "❌ Typecheck falhou"; exit 1; }
pnpm build || { echo "❌ Build falhou"; exit 1; }

echo "✅ Quick Wins aplicados e validados com sucesso!"
echo "Pode prosseguir com 'git add . && git commit -m \"feat: apply perf quick wins (timeout, singleton, seed)\"'"
EOF
