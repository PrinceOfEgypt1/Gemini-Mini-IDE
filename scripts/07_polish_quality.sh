...
# ==============================================================================
# SCRIPT: 07_polish_quality.sh
# DESCRIÇÃO: 
#   1. Elimina warnings do ESLint em cache.service.ts (Regra no-console).
#   2. Valida pipeline com critério 'max-warnings 0'.
# AUTOR: Mini-IDE Engine Team
# ==============================================================================

echo ">>> Iniciando Polimento de Qualidade (Zero Warnings)..."

# ------------------------------------------------------------------------------
# 1. Corrigindo packages/analysis-agent/src/services/cache.service.ts
# ------------------------------------------------------------------------------
echo ">>> Aplicando correções de lint em packages/analysis-agent/src/services/cache.service.ts..."
...
# ------------------------------------------------------------------------------
# 2. Verificação Rigorosa (Zero Warnings)
# ------------------------------------------------------------------------------
echo ">>> Executando validação rigorosa..."

# Lint com flag --max-warnings 0 para garantir tolerância zero
echo ">>> [Check] ESLint (Max Warnings: 0)..."
pnpm lint --filter @mini-ide/analysis-agent --max-warnings 0 || { echo "❌ Lint falhou com warnings"; exit 1; }

echo ">>> [Check] Typecheck..."
pnpm typecheck --filter @mini-ide/analysis-agent || { echo "❌ Typecheck falhou"; exit 1; }

echo ">>> [Check] Build..."
pnpm build --filter @mini-ide/analysis-agent || { echo "❌ Build falhou"; exit 1; }
