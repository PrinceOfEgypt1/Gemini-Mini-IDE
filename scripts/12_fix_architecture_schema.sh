...
# ==============================================================================
# SCRIPT: 12_fix_architecture_schema.sh
# DESCRIÇÃO: 
#   1. Relaxa o Zod Schema de Architecture (Criticality vira string livre).
#   2. Adiciona logs de debug profundos para inspecionar o JSON bruto em caso de falha.
# AUTOR: Mini-IDE Engine Team
# ==============================================================================

echo ">>> Iniciando Correção de Schema de Arquitetura (Fase 12)..."
...
// Alterado de Union Type restrito para string para aceitar "Test", "Docs", etc.
...
// CORREÇÃO CRÍTICA: Criticality agora é z.string() para não quebrar com "Test" ou "Infrastructure"
...
function sanitizeGeneric<T>(raw: unknown, schema: z.ZodType<T>, fallback: T, contextName: string): T {
  try {
    ...
    if (typeof raw === 'object' && raw !== null) {
      ...
      // Log do payload problemático para debug
      // eslint-disable-next-line no-console
      console.warn(`[Agent] Payload Failed (${contextName}):`, JSON.stringify(raw).substring(0, 500) + "...");
    } else {
       // eslint-disable-next-line no-console
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, _budgetContext?: BudgetContext): Promise<AgentResult> {
    // eslint-disable-next-line no-console
    console.info(`[Agent v12.0] Criticality Schema Relaxed. Cache Size: ${globalAnalysisCache.stats().size}`);
    ...
    try {
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Generated via Agent v12.0" }
      };
  ...
  private async generateFileContent(spec: ManifestItem, stack: string, pattern: string, userPrompt: string): Promise<GeneratedFile> {
    try {
      ...
      const techSpec = await this.callLLM(SYSTEM_PROMPTS.TECH_SPEC, promptContext, (r) => sanitizeGeneric(r, TechnicalSpecSchema, { file_path: spec.path, technical_spec: { imports_required: [], interfaces_to_define: [], functions_to_implement: [] } }, "TechSpec"), TechnicalSpecSchema, `Spec:${spec.path}`);
...
echo "✅ Fase 12 (Schema Relaxado) concluída."
echo "Agora o Arquiteto tem liberdade para categorizar arquivos sem quebrar o pipeline."
EOF
