...
# ==============================================================================
# SCRIPT: 14_final_release_candidate.sh
# DESCRIÇÃO: Consolidação Final (Gold Master).
#   - Prompts: Fase 13 (Inferência Estrita).
#   - Agente: Fase 12 (Resiliência, Timeout 120s, Backoff).
#   - Cache: Fase 7 (Persistente em Disco).
# AUTOR: Mini-IDE Engine Team
# ==============================================================================

echo ">>> Aplicando Versão Final (Release Candidate)..."

# ------------------------------------------------------------------------------
# 1. Prompts (Fase 13 - Rigor Semântico)
# ------------------------------------------------------------------------------
echo ">>> [1/3] Atualizando Prompts (Rigor Semântico)..."
...
JSON Estrito: { "type": "...", "reasoning": "..." }
`.trim(),

  ANALYSIS: `
Você é um Engenheiro de Requisitos Principal.
Sua tarefa CRÍTICA é limpar e estruturar o pedido do usuário.

ENTRADA: O texto pode vir sujo, copiado de PDFs, com bullets (o, ) e quebras de linha erradas.
AÇÃO:
1. Ignore completamente a formatação quebrada.
2. Identifique o OBJETIVO REAL do sistema (Domínio de Negócio).
3. Reescreva o resumo em linguagem natural fluida e profissional.

Retorne JSON estrito PT-BR:
{
  "summary": "Texto fluido descrevendo o sistema limpo",
  "complexity": "Alta",
...
Defina Épicos baseados estritamente na Análise de Requisitos.

Diretrizes:
1. Cubra o ciclo de vida completo dos dados identificados.
2. Utilize terminologia profissional adequada ao domínio.
  ...
  "epics": [
    {
      ...
      "context": "Justificativa de negócio",
      "requirements": ["Requisito funcional explícito"]
    }
  ]
}
`.trim(),
...
Defina a estrutura de CÓDIGO-FONTE (src/).

Regras:
1. Stack: React + Vite (Front) e Node + Fastify (Back).
2. Padrão: Clean Architecture ou MVC Modular.
3. Se o sistema for complexo, crie pastas separadas para controllers, services, models.
...
`.trim(),
  ...
  QA_STRATEGY: `
Você é um QA Lead.
Gere o manifesto de testes.
Regra: Para cada Service ou Controller crítico, deve haver um arquivo de teste correspondente.
Retorne JSON estrito: { "manifest": [...] }
`.trim(),

  DEVOPS_STRATEGY: `
Você é um DevOps Engineer.
Gere o manifesto de infraestrutura padrão.
Inclua: package.json, tsconfig.json, docker-compose.yml, .gitignore, README.md.
Retorne JSON estrito: { "manifest": [...] }
`.trim(),

  CODE_GEN: `
Você é um Desenvolvedor Sênior Especialista.
Implemente o código para o arquivo solicitado.

DIRETRIZES DE IMPLEMENTAÇÃO (STRICT):
1. **Contexto de Domínio**: O código deve refletir o negócio (nomes de métodos e variáveis reais).
2. **Sem Placeholders**: É PROIBIDO usar comentários como "// lógica aqui". Escreva a lógica padrão.
3. **Tratamento de Erros**: Inclua try/catch e validação.
...
Não invente dados aleatórios. **Derive** a ação e o benefício a partir do NOME DO ÉPICO e do contexto do sistema.
- Errado: "Como usuário, quero realizar ação..." (Genérico)
- Correto: "Como [Papel do Domínio], quero [Ação Específica], para [Valor de Negócio]."
...
Retorne JSON estrito PT-BR:
{
  "userStories": [
    {
      "id": "HU-001",
      "title": "Título Específico do Domínio",
      "priority": "P0",
      "role": "Papel Específico",
      "action": "Ação concreta no sistema",
      "benefit": "Valor de negócio claro",
      "acceptanceCriteria": ["Dado [cenário], Quando [ação], Então [resultado]"],
...
# ------------------------------------------------------------------------------
# 2. Agente (Fase 12 - Resiliência)
# ------------------------------------------------------------------------------
echo ">>> [2/3] Atualizando Agente (Resiliência de Rede)..."
...
const StrategySchema = z.object({ manifest: z.array(ManifestItemSchema) });
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, _budgetContext?: BudgetContext): Promise<AgentResult> {
    ...
    logger.info(`[Agent v14.0] Gold Master. Cache Size: ${globalAnalysisCache.stats().size}`);
    ...
    try {
      ...
      logger.info("[Agent] Arquiteto definindo estrutura...");
      ...
      return {
        ...
        fenix: { notes: "Generated via Agent v14.0 (Release Candidate)" }
      };
  ...
  private async generateTextResponse(prompt: string): Promise<string> {
      const completion = await this.client.chat.completions.create({ model: this.model, messages: [{ role: "user", content: prompt }] }, { timeout: 120000 });
  ...
  private async runQAStep(arch: Architecture): Promise<StrategyManifest> { const context = `Stack: ${arch.stack}\nArquivos Propostos: ${arch.manifest.map(m=>m.path).join(", ")}`; return this.callLLM(SYSTEM_PROMPTS.QA_STRATEGY, context, sanitizeStrategy, StrategySchema, "QA Strategy"); }
  private async runDevOpsStep(arch: Architecture): Promise<StrategyManifest> { const context = `Stack: ${arch.stack}\nArquivos: ${arch.manifest.length}`; return this.callLLM(SYSTEM_PROMPTS.DEVOPS_STRATEGY, context, sanitizeStrategy, StrategySchema, "DevOps Strategy"); }
  private async generateFileContent(spec: ManifestItem, stack: string, userPrompt: string): Promise<GeneratedFile> {
    ...
    } catch (error) { return { path: normalizePath(spec.path), content: "// Error: Falha definitiva após 3 tentativas.", language: "plaintext" }; }
  }
  ...
  private async callLLM<T>(sys: string, usr: string, san: SanitizeFunction<T>, sch: z.ZodType<T>, ctx: string): Promise<T> {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn(`[Agent] Falha ${attempt}/${maxRetries} em ${ctx}. Erro: ${err}. Retrying in ${delay}ms...`);
...
# ------------------------------------------------------------------------------
# 3. Limpeza Final de Cache
# ------------------------------------------------------------------------------
echo ">>> [3/3] Limpando Cache para novo Teste..."
...
echo "✅ Versão Final (Fase 14) Aplicada com Sucesso."
