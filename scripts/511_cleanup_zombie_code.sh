...
log_info "Iniciando remoção de código zumbi e consolidação do Agente..."

# 1. REMOVER CÓDIGO MORTO
# A nova arquitetura (v3.0+) concentra a lógica no agent.ts.
# As pastas 'core' e 'modules' são vestígios de tentativas anteriores.
rm -rf "$BASE_DIR/core"
rm -rf "$BASE_DIR/modules"
log_ok "Pastas obsoletas (core, modules) removidas."

# 2. RE-APLICAR O AGENTE (V4.1 - Heavy Duty + Content Quality + Fixes)
# Garantimos que o arquivo em disco é EXATAMENTE o que queremos.
cat > "$BASE_DIR/agent.ts" << 'EOF'
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, budgetContext: unknown): Promise<unknown> {
    console.log("🚀 [Agent v4.2] Iniciando Pipeline (Zombie-Free)");
    ...
    try {
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Generated via v4.2 Clean Agent" }
      };
  ...
  private async runArchitectureStep(prompt: string, productPlan: any) {
    ...
    return this.callLLM(`Arquiteto. PT-BR. Lista COMPLETA de arquivos. JSON: {stack, manifest:[{path, purpose, criticality}]}`, `Arquitetura para: ${context}`, sanitizeArchitecture, ArchitectureSchema, "Architecture");
  }
  private async generateFileContent(spec: ManifestItem, stack: string, prompt: string) {
    try {
        ...
        return { path: normalizePath(spec.path), content: \`// Erro: \${error.message}\`, language: this.detectLanguage(spec.path) };
    }
  }
  private async expandEpicsToStories(epics: any[]) {
    ...
    return (await this.callLLM(`PO Técnico. PT-BR. HUs Detalhadas. JSON: {userStories:[...]}`, `Contexto: ${richContext}`, sanitizeUserStories, UserStoriesSchema, "HUs")).userStories;
  }
...
EOF
log_ok "Agent restaurado e limpo."

# 3. RE-APLICAR OS TESTES (Compatíveis com v4.2)
cat > "packages/analysis-agent/src/index.test.ts" << 'EOF'
...
log_ok "Limpeza concluída. Reinicie o servidor."
