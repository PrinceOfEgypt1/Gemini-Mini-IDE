...
echo "🚒 Sprint 8.6: Apagando o incêndio no Smoke Test (Corrigindo paths)..."

# Recria o script de checklist com o caminho atualizado para dist/src/index.js
cat > 42_pipeline_checklist.sh <<'EOF'
#!/usr/bin/env bash
################################################################################
# Script: 42_pipeline_checklist.sh
# Versão: 1.2.0 (Path Fix for TS NodeNext Structure)
# Data: 2024-11-22
################################################################################
...
if [[ "$SKIP_SERVER_START" == "0" ]]; then
  ...
  # FIX: Caminho atualizado para dist/src/index.js devido à mudança de rootDir
...
echo "✅ Checklist atualizado para nova estrutura de pastas."
echo "👉 Execute ./42_pipeline_checklist.sh para confirmar a extinção do incêndio."
