...
# ==============================================================================
# SCRIPT: 11_fix_compilation_errors.sh
# DESCRIÇÃO: 
#   1. Remove ManifestItemSchema não utilizado (Erro TS6133).
#   2. Utiliza o parâmetro 'pattern' no prompt de geração (Erro TS6133).
#   3. Valida a build.
# AUTOR: Mini-IDE Engine Team
# ==============================================================================

echo ">>> Iniciando Correção de Compilação (Fase 11)..."

echo ">>> Corrigindo packages/analysis-agent/src/agent.ts..."
...
// ManifestItemSchema removido pois não era utilizado (causava erro TS6133)
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, _budgetContext?: BudgetContext): Promise<AgentResult> {
    // eslint-disable-next-line no-console
    console.info(`[Agent v10.1] Compilation Fix. Cache Size: ${globalAnalysisCache.stats().size}`);
    ...
    try {
      ...
      return {
        ...
        fenix: { notes: "Generated via Agent v10.1" }
      };
  ...
  private async generateFileContent(spec: ManifestItem, stack: string, pattern: string, userPrompt: string): Promise<GeneratedFile> {
    try {
      ...
      // CORREÇÃO: 'pattern' agora é utilizado na string de prompt, resolvendo o erro TS6133
...
echo "✅ Compilação Corrigida (Phase 11)."
EOF
