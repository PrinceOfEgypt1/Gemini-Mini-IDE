...
# ------------------------------------------------------------------------------
# Script: scripts/212_fix_strict_hu_card.sh
# Objetivo: Corrigir erro de sintaxe no UserStoryCard.tsx (Backticks e Regex)
# Autor: Mini-IDE Agent
# Data: 2025-11-24
# ------------------------------------------------------------------------------

echo "🔧 Corrigindo sintaxe do UserStoryCard.tsx..."

cat << 'EOF' > packages/ui/src/components/hus/UserStoryCard.tsx
import React, { useState, useMemo } from 'react';

// Interface completa baseada no Padrão de Excelência
...
export const UserStoryCard: React.FC<UserStoryCardProps> = ({ story }) => {
  ...
  const structuredData = useMemo(() => {
    ...
    const extractSection = (header: string) => {
      // Regex busca por "## Header" ou "** Header" até o próximo cabeçalho ou fim do texto
      // Nota: Não escapamos crases aqui pois o cat usa 'EOF'
      const regex = new RegExp(`(?:##|\\*\\*)\\s*${header}[\\s\\S]*?(?=(?:##|\\*\\*|$))`, 'i');
      const match = rawText.match(regex);
      
      if (!match) return null;
      
      // Remove o próprio cabeçalho do texto encontrado para retornar apenas o conteúdo
    ...
    return {
      ...
      // Se não vier estruturado, assumimos arrays vazios por enquanto
      rf: story.functionalReqs || [],
...
echo "✅ Correção de sintaxe aplicada com sucesso."
EOF
