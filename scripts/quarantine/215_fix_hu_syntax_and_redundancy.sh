...
# ------------------------------------------------------------------------------
# Script: scripts/215_fix_hu_syntax_and_redundancy.sh
# Objetivo: Corrigir erro de sintaxe (escape incorreto) e aplicar filtro de redundância
# Autor: Mini-IDE Agent
# Data: 2025-11-24
# ------------------------------------------------------------------------------

echo "🔧 Corrigindo sintaxe do UserStoryCard e removendo redundâncias..."
...
export const UserStoryCard: React.FC<UserStoryCardProps> = ({ story }) => {
  ...
  const structuredData = useMemo(() => {
    ...
    const extractSection = (header: string): string | null => {
      // Regex corrigida: sem escape na crase
    ...
    // Encontra o "Para" (Benefit) primeiro
    ...
    // Remove trechos que começam com "para ..." dentro da ação, pois isso pertence ao benefício
    const cleanAction = rawAction
      ? rawAction
          .replace(/\s+para\s+que\s+.*$/i, '') 
          .replace(/\s+para\s+o\s+objetivo.*$/i, '')
          .replace(/\s+para\s+poder\s+.*$/i, '')
          .trim()
...
echo "✅ UserStoryCard corrigido (Sintaxe e Redundância)."
EOF
