...
log_info "Desbloqueando geração completa e forçando PT-BR no Agent..."

# Reescreve agent.ts com as correções finais
cat > "$AGENT_FILE" << 'EOF'
import { OpenAI } from "openai";
import { z } from "zod";

// TIPOS & SCHEMAS (Mantidos, apenas reforçando prompts abaixo)
...
// HELPER: Limpeza de JSON markdown
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, budgetContext: unknown): Promise<unknown> {
    console.log("🚀 [Agent v3.5] Iniciando Pipeline (Unlimited Generation)");
    ...
    try {
      ...
      // CORREÇÃO CRÍTICA: REMOÇÃO DO LIMITE DE 6 ARQUIVOS
      // Agora processamos TODOS os arquivos do manifesto, em lotes de 5 para não estourar conexões
      const batchSize = 5;
      const allFiles: any[] = [];

      console.log(`🏭 [4/4] Iniciando geração de TODOS os ${manifest.length} arquivos...`);

      for (let i = 0; i < manifest.length; i += batchSize) {
        const batch = manifest.slice(i, i + batchSize);
        console.log(`   Batch ${Math.floor(i/batchSize) + 1}: Gerando arquivos ${i + 1} a ${i + batch.length}...`);
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Generated via v3.5 Unleashed Agent" }
      };
...
Defina a stack e arquivos para: ${context}.
Responda APENAS JSON.
IMPORTANTE:
1. Use caminhos relativos (ex: "src/index.ts").
2. Gere arquivos suficientes para um MVP funcional (mínimo 10).
3. Inclua configurações (package.json, tsconfig.json).
JSON: { "stack": "string", "manifest": [{ "path": "string", "purpose": "string", "criticality": "Core"|"Support"|"Config" }] }`;
  ...
  private async generateFileContent(spec: ManifestItem, stack: string, prompt: string) {
    try {
        // Prompt otimizado para código real
        const systemPrompt = `Dev Sênior. Implemente ${spec.path}. Stack: ${stack}.
Gere código COMPLETO, funcional e sem "..." ou placeholders.
...
Responda JSON: { "path": "${spec.path}", "code": "...", "explanation": "..." }`;
  ...
  private async expandEpicsToStories(epics: any[]) {
    ...
    const systemPrompt = `PO Técnico. Responda em PORTUGUÊS DO BRASIL.
JSON: { "userStories": [...] }.
Campos obrigatórios (preencha todos): id, title, priority, role, action, benefit, acceptanceCriteria[], functionalRequirements[], securityRequirements[], businessContext.`;
  ...
  private async callLLM<T>(sys: string, usr: string, san: SanitizeFunction<T>, sch: z.ZodType<T>, ctx: string): Promise<T> {
    try {
      ...
      try {
        rawData = JSON.parse(cleanJsonString(rawContent));
      } catch {
        console.warn(`⚠️ [${ctx}] JSON Parse Error. Tentando recuperação...`);
        rawData = {};
      }
...
EOF
log_ok "Agent v3.5 (Unleashed & PT-BR) aplicado."
...
log_ok "Pronto. Servidor reiniciado. A geração agora será COMPLETA (pode demorar)."
