...
log_info "Eliminando o último 'any' em DocsPanel.tsx..."

# Reescreve o arquivo com a tipagem 'unknown' em vez de 'any'
cat > "$DOCS_PATH" << 'EOF'
...
// FIX: 'node' tipado como 'unknown' para satisfazer no-explicit-any
    ...
    <div className="h-full flex flex-col bg-[var(--bg-app)] rounded-lg border border-[var(--border-main)]">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <article className="prose prose-invert max-w-none text-sm text-[var(--text-secondary)]">
          <ReactMarkdown
            components={{
              // 'node' agora é unknown, mas como renomeamos para _node (não usado), o TS aceita
...
# Executar a pipeline para confirmar o sucesso total
