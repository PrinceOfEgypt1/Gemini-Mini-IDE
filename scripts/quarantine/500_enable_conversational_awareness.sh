...
log_info "Implementando Router de Intenção no Agente..."

# Reescreve agent.ts com Router de Intenção
cat > "$AGENT_FILE" << 'EOF'
...
// NOVO SCHEMA: Router de Intenção
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, budgetContext: unknown): Promise<unknown> {
    console.log("🚀 [Agent v5.0] Iniciando Análise de Intenção...");

    try {
      // PASSO 0: Router de Intenção
      const intent = await this.detectIntent(userPrompt);
      console.log(`🎯 Intenção Detectada: ${intent.type} (${intent.reasoning})`);

      // Se for apenas uma pergunta ou conversa, não gera projeto novo
      if (intent.type === "QUESTION") {
        // Responde apenas com o texto no summary e objetos vazios
        ...
        return {
           ...
           engine: { files: [] }, // Arquivos vazios = UI não atualiza a árvore
           ux: { components: [] },
           quality: { tests: [] },
           ops: { scripts: [] },
      ...
      // Se for NEW_PROJECT ou REFINEMENT, roda o pipeline completo
      // (Futuramente REFINEMENT pode rodar pipeline parcial)
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Generated via v5.0 Conversational Agent" }
      };
  ...
  // --- NOVO: Router de Intenção ---
  private async detectIntent(prompt: string) {
    const systemPrompt = `Classifique a intenção do usuário.
    - NEW_PROJECT: "Crie um app...", "Gere uma API...", "Quero um sistema..."
    - QUESTION: "Por que não tem docs?", "Como funciona?", "O que é React?"
    - REFINEMENT: "Mude a cor para azul", "Adicione testes", "Use Postgres"
    
    Responda JSON: { "type": "NEW_PROJECT" | "QUESTION" | "REFINEMENT", "reasoning": "string" }`;
    
    // Schema Zod local para intent
    const IntentZod = z.object({
        type: z.enum(["NEW_PROJECT", "QUESTION", "REFINEMENT"]),
        reasoning: z.string()
    });

    return this.callLLM(systemPrompt, prompt, (r: any) => r, IntentZod, "Intent Detection");
  }

  // --- NOVO: Resposta de Texto Simples ---
  private async generateTextResponse(prompt: string) {
    const completion = await this.client.chat.completions.create({
        model: "gpt-4o",
        messages: [
          { role: "system", content: "Você é um assistente de engenharia. Responda à dúvida do usuário sobre o projeto ou tecnologia de forma concisa e útil." },
...
Responda JSON: { "stack": "string", "manifest": [{ "path": "string", "purpose": "string", "criticality": "Core"|"Support"|"Config" }] }`;
    ...
    return this.callLLM(systemPrompt, `Arquitetura para: ${context}`, sanitizeArchitecture, ArchitectureSchema, "Architecture");
  }
...
Responda JSON: { "path": "${spec.path}", "code": "...", "explanation": "..." }`;
  ...
  private async expandEpicsToStories(epics: any[]) {
    ...
    const richContext = targetEpics.map((e: any, idx: number) => `ÉPICO ${idx+1}: ${e.title}\nCONTEXTO: ${e.context}\nREQUISITOS: ${e.requirements.join("; ")}`).join("\n---\n");

    const systemPrompt = `PO Técnico Sênior. Responda em PORTUGUÊS DO BRASIL.
    JSON: { "userStories": [...] }.
    Preencha TODOS os campos obrigatórios (id, title, priority, role, action, benefit, acceptanceCriteria[], functionalRequirements[], securityRequirements[], businessContext).`;
...
EOF
log_ok "Agent v5.0 (Conversational Router) aplicado."
