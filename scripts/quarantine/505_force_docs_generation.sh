...
log_info "Aplicando Enforcement de Documentação (README.md) no Agent..."

# Reescreve o agent.ts com a lógica de injeção de README
cat > "$AGENT_FILE" << 'EOF'
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, context?: ProjectContext): Promise<unknown> {
    console.log("🚀 [Agent v6.2] Iniciando Análise (Docs Enforced)");
  ...
  private async runFullPipeline(userPrompt: string) {
      ...
      // --- DOCS ENFORCEMENT ---
      // Verifica se o README.md foi planejado. Se não, injeta na marra.
      const hasReadme = manifest.some(f => f.path.toLowerCase().includes('readme.md'));
      if (!hasReadme) {
          console.log("⚠️ [Agent] README faltando. Injetando no manifesto...");
          manifest.unshift({
              path: "README.md",
              purpose: "Documentação completa do projeto, como rodar e instalar.",
              criticality: "Config"
          });
      }
      // -----------------------
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Full Generation v6.2" }
      };
  }
  ...
  private async runArchitectureStep(prompt: string, productPlan: any) {
    ...
    // PROMPT REFORÇADO + OBRIGATORIEDADE DE README
    const systemPrompt = `Arquiteto Sênior.
    Defina stack e arquivos. Contexto: ${context}.
    REGRAS:
    1. Use caminhos REAIS (ex: "src/index.ts").
    2. Cubra todas as camadas.
    3. OBRIGATÓRIO: Incluir "README.md" com documentação detalhada.
  ...
  private async generateFileContent(spec: ManifestItem, stack: string, prompt: string) {
    try {
        ...
        Para README.md, use Markdown rico.
...
log_ok "Agent v6.2 (Docs Enforcement) aplicado."
