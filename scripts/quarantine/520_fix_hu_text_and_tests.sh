...
HU_CARD_FILE="packages/ui/src/components/hus/UserStoryCard.tsx"
AGENT_FILE="packages/analysis-agent/src/agent.ts"

log_info "Iniciando correção de HUs duplicadas e geração de testes..."

# 1. CORRIGIR DUPLICAÇÃO DE TEXTO NAS HUs (Frontend)
log_info "Aplicando limpeza de prefixos no UserStoryCard.tsx..."

# Vamos injetar uma função de limpeza dentro do componente
cat > "$HU_CARD_FILE" << 'EOF'
...
export const UserStoryCard: React.FC<UserStoryCardProps> = ({ story }) => {
  ...
  const structuredData = useMemo(() => {
    ...
    let action = cleanPrefix(story.action, "Quero");
    let benefit = cleanPrefix(story.benefit, "Para");
...
EOF
log_ok "Frontend corrigido (HUs limpas)."

# 2. FORÇAR GERAÇÃO DE TESTES (Backend Agent)
log_info "Reforçando prompt do Arquiteto para incluir testes..."

# Vamos injetar uma regra explícita sobre testes no prompt do Arquiteto
sed -i 's/2. Inclua src\/controllers, src\/services, src\/models, src\/utils, tests\/, config\/.*/2. Inclua src\/controllers, src\/services, src\/models. OBRIGATÓRIO: Gere pelo menos 3 arquivos de teste na pasta "tests\/" ou "__tests__\/"./' "$AGENT_FILE"

log_ok "Agente configurado para exigir testes."
