...
log_info "Aplicando Protocolo de Determinismo (Temperatura 0 + Clean Architecture)..."

# Reescreve o agent.ts com configurações estritas
cat > "$AGENT_FILE" << 'EOF'
...
// Helper para limpar prefixos de HUs no Backend
function stripPrefix(text: string, prefix: string): string {
    const regex = new RegExp(`^(${prefix}|como|quero|para)\\s*[:,-]?\\s*`, 'i');
...
function sanitizeUserStory(raw: unknown, index: number): UserStory {
  ...
  const rawRole = ensureString(story.role || story.actor, "usuário");
  const rawAction = ensureString(story.action || story.goal, "realizar ação");
  const rawBenefit = ensureString(story.benefit || story.reason, "obter valor");
  ...
  return {
    ...
    // LIMPEZA DE PREFIXOS AQUI
    role: stripPrefix(rawRole, "como"),
    action: stripPrefix(rawAction, "quero"),
    benefit: stripPrefix(rawBenefit, "para"),
...
interface ProjectContext { files: Array<{ path: string; purpose?: string }>; summary?: string; }
...
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, context?: ProjectContext): Promise<unknown> {
    console.log("🚀 [Agent v7.0 Deterministic] Iniciando Pipeline");
  ...
  private buildEmptyResponse(summary: string, notes: string) {
      return { summary, requestId: `chat-${Date.now()}`, timestamp: new Date().toISOString(), analysis: { summary, complexity: "Baixa", assumptions: [] }, product: { userStories: [] }, architect: { stack: "", diagram: "" }, engine: { files: [] }, ux: { components: [] }, quality: { tests: [] }, ops: { scripts: [] }, fenix: { notes } };
  }
  ...
  private async runFullPipeline(userPrompt: string) {
      ...
      return {
        ...
        quality: { tests: [] },
        ops: { scripts: [] },
        fenix: { notes: "Deterministic Generation v7.0" }
      };
  }
  ...
  private async runRefinementPipeline(prompt: string, context: ProjectContext) {
      ...
      return { summary: `Refinamento: ${prompt}`, requestId: `ref-${Date.now()}`, timestamp: new Date().toISOString(), analysis: { summary: "Refinamento", complexity: "Média", assumptions: [] }, product: { userStories: [] }, architect: { stack: changes.stack, diagram: "" }, engine: { files: allFiles }, ux: { components: [] }, quality: { tests: [] }, ops: { scripts: [] }, fenix: { notes: "Refinement" } };
  }

  private async detectIntent(prompt: string) {
    const systemPrompt = `Classifique a intenção. JSON: { "type": "NEW_PROJECT"|"QUESTION"|"REFINEMENT", "reasoning": "string" }`;
    const IntentZod = z.object({ type: z.enum(["NEW_PROJECT", "QUESTION", "REFINEMENT"]), reasoning: z.string() });
    // Temperatura 0.0 para classificação precisa
    return this.callLLM(systemPrompt, prompt, (r: any) => r, IntentZod, "Intent", 0.0);
  }

  private async generateTextResponse(prompt: string, context?: ProjectContext) {
    let sys = "Assistente técnico Mini-IDE.";
    if (context?.files?.length) sys += `\nArquivos:\n${context.files.map(f => f.path).join("\n")}`;
  ...
  private async runAnalysisStep(prompt: string) {
    return this.callLLM(`Analista. PT-BR. JSON: {summary, complexity, assumptions[]}`, prompt, sanitizeAnalysis, AnalysisSchema, "Analysis", 0.0);
  }
  private async runProductStep(prompt: string, analysis: any) {
    return this.callLLM(`PO. PT-BR. JSON: {epics:[{title, context, requirements[]}]}`, prompt, sanitizeProductPlan, ProductPlanSchema, "Product", 0.0);
  }
  
  private async runArchitectureStep(prompt: string, productPlan: any) {
    const context = productPlan.epics.map((e:any)=>e.title).join(", ");
    // PROMPT ARQUITETURAL RÍGIDO
    const systemPrompt = `Arquiteto Sênior. Stack: Node/React/TS.
    Defina a estrutura seguindo CLEAN ARCHITECTURE rigorosamente.
    
    OBRIGATÓRIO GERAR ESTES GRUPOS DE ARQUIVOS:
    1. Config: package.json, tsconfig.json, README.md, .gitignore, vite.config.ts, jest.config.js
    2. Backend: src/server/index.ts, src/server/app.ts, src/server/routes.ts
    3. Backend Layers: Separe controllers, services, repositories, models (ex: src/server/modules/user/user.controller.ts)
    4. Frontend: src/client/main.tsx, src/client/App.tsx
    5. Tests: Crie pelo menos 3 arquivos .test.ts na pasta tests/
    
    NÃO ECONOMIZE. GERE A LISTA COMPLETA (~30 arquivos).
    Responda JSON: { "stack": "string", "manifest": [{ "path": "string", "purpose": "string", "criticality": "Core"|"Support"|"Config" }] }`;
    
    return this.callLLM(systemPrompt, `Contexto: ${context}`, sanitizeArchitecture, ArchitectureSchema, "Architecture", 0.0);
  }

  private async generateFileContent(spec: ManifestItem, stack: string, prompt: string) {
    try {
        // Prompt para código determinístico
        const systemPrompt = `Dev Sênior. Implemente ${spec.path}. Stack: ${stack}.
        Regras:
        1. Código completo e funcional.
        2. Sem "..." ou placeholders.
        3. Siga Clean Code.
        4. PT-BR nos comentários.
        JSON: { "path": "${spec.path}", "code": "...", "explanation": "..." }`;
        const parsed = await this.callLLM(systemPrompt, "Codifique.", (r) => sanitizeFileContent(r, spec.path), FileContentSchema, `File:${spec.path}`, 0.0);
  ...
  private async expandEpicsToStories(epics: any[]) {
    ...
    const systemPrompt = `PO Técnico. HUs. PT-BR.
    JSON: { "userStories": [{ "id": "HU-001", "title": "...", "priority": "P0"|"P1", "role": "...", "action": "...", "benefit": "...", "acceptanceCriteria": ["..."], "functionalRequirements": ["..."], "securityRequirements": ["..."], "businessContext": "..." }] }.
    Preencha TUDO. NÃO use prefixos "Como", "Quero", "Para" nos campos role/action/benefit (apenas o conteúdo).`;

    const result = await this.callLLM(systemPrompt, `Contexto: ${ctx}`, sanitizeUserStories, UserStoriesSchema, "HUs", 0.0);
    return result.userStories;
  }

  private async callLLM<T>(sys: string, usr: string, san: SanitizeFunction<T>, sch: z.ZodType<T>, ctx: string, temp: number = 0.0): Promise<T> {
    try {
      const c = await this.client.chat.completions.create({ 
          model: "gpt-4o", 
          messages: [{role:"system",content:sys},{role:"user",content:usr}], 
          response_format:{type:"json_object"}, 
          temperature: temp // DETERMINISMO TOTAL (0.0)
      });
...
log_ok "Agent v7.0 (Zero Temp + Clean Arch) aplicado."
