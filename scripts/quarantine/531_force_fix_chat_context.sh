...
log_info "Reescrevendo agent.ts com lógica de Contexto de Chat (v5.1)..."

# Reescreve o arquivo completo para evitar erros de regex
cat > "$AGENT_FILE" << 'EOF'
...
// Interfaces do Contexto (Importante para a memória)
...
/**
 * AnalysisAgent v5.1 - Context Aware Chat
 */
export class AnalysisAgent {
  ...
  async analyze(userPrompt: string, context?: ProjectContext): Promise<unknown> {
    console.log("🚀 [Agent v5.1] Iniciando Análise Contextual...");
    ...
    try {
      ...
      // 2. Fluxo de Chat (QUESTION) - Agora COM contexto
      if (intent.type === "QUESTION") {
        // Passa o contexto completo para gerar a resposta textual
        ...
        return {
           ...
           // Retorna estruturas vazias para manter o estado do frontend inalterado
           ...
           quality: { tests: [] },
           ops: { scripts: [] },
           fenix: { notes: "Context Aware Chat Response" }
        };
      }

      // 3. Fluxo de Refinamento (REFINEMENT)
      if (intent.type === "REFINEMENT" && context && context.files.length > 0) {
          console.log("🔄 [Refinement] Usando contexto existente para alterar projeto...");
          return this.runRefinementPipeline(userPrompt, context);
      }

      // 4. Fluxo de Novo Projeto (NEW_PROJECT)
      console.log("✨ [New Project] Iniciando geração completa...");
  ...
  // Gera resposta textual consciente do projeto
  private async generateTextResponse(prompt: string, context?: ProjectContext) {
    let systemContext = "Você é um assistente técnico sênior do Mini-IDE.";
    
    if (context && context.files && context.files.length > 0) {
        const fileList = context.files.map(f => f.path).join(", ");
        // Injeta o contexto do projeto no system prompt
        systemContext += `\n\nCONTEXTO DO PROJETO ATUAL:\nResumo: ${context.summary || "N/A"}\nArquivos Existentes: ${fileList}\n\nINSTRUÇÃO: Responda à pergunta do usuário com base neste projeto. Se ele pedir para ver algo, indique em qual arquivo está.`;
    } else {
  ...
  private async detectIntent(prompt: string) {
    ...
    - NEW_PROJECT: "Crie um app", "Gere uma API". (Criar do zero)
    - QUESTION: "Por que não tem docs?", "Mostre os arquivos", "Explique o código". (Dúvida sobre o atual)
    - REFINEMENT: "Adicione testes", "Crie a documentação", "Mude para azul". (Alterar o atual)
  ...
  private async runRefinementPipeline(prompt: string, context: ProjectContext) {
      ...
      Se for pedido "documentação", crie README.md ou docs/arquivos.md.
      
      Responda JSON: { "stack": "manter", "manifest": [{ "path": "...", "purpose": "...", "criticality": "Core" }] }`;
      
      const changes = await this.callLLM(systemPrompt, "Planeje.", sanitizeArchitecture, ArchitectureSchema, "Refinement Arch");
      ...
      return {
          summary: `Refinamento concluído: ${prompt}\nArquivos gerados/alterados: ${allFiles.map(f => f.path).join(", ")}`,
  ...
  private async runArchitectureStep(prompt: string, productPlan: any) {
    ...
    REGRAS: NÃO ECONOMIZE ARQUIVOS. Inclua src/, tests/, config/.
  ...
  private async generateFileContent(spec: ManifestItem, stack: string, prompt: string) {
    try {
        const systemPrompt = `Dev. Codifique ${spec.path}. Stack: ${stack}.
        Gere código COMPLETO. Comentários em PT-BR.
...
log_ok "Agent v5.1 (Contexto de Chat e Refinamento) aplicado."
...
log_ok "Correção aplicada. O Agente agora lembra dos arquivos ao responder perguntas."
