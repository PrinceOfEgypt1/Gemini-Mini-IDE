...
echo "🚒 Sprint 8.7: Eliminando scripts de checklist duplicados e corrigindo caminho..."

# 1. Remover a versão antiga/incorreta dentro de scripts/
if [ -f "scripts/42_pipeline_checklist.sh" ]; then
    echo "🗑️ Removendo script obsoleto em scripts/42_pipeline_checklist.sh..."
    rm scripts/42_pipeline_checklist.sh
fi

# 2. Recriar o script na RAIZ (Local Correto) com o caminho do servidor atualizado
echo "📝 Recriando ./42_pipeline_checklist.sh na raiz..."
...
echo "🔍 Verificando estrutura de build do servidor..."
if [ -f "packages/server/dist/src/index.js" ]; then
    echo "✅ Arquivo de inicialização encontrado: packages/server/dist/src/index.js"
else
    echo "⚠️  AVISO: O arquivo packages/server/dist/src/index.js não foi encontrado."
    echo "   Isso pode ocorrer se o 'pnpm build' ainda não rodou com a nova config."
    echo "   O pipeline vai tentar construir e deve resolver isso."
fi

echo "✅ Script de correção executado."
echo "👉 Execute AGORA o comando abaixo (da RAIZ):"
echo "   ./42_pipeline_checklist.sh"
